...
public static class UnityExtensions
{
	...
	public static byte[] UnityStringToBytesASCII(this string source)
	{
		// exit if null
		...
		{
			using (StreamWriter writer = new StreamWriter(compMemStream, Encoding.ASCII))
			{
				writer.Write(source);
				writer.Close();
	...
	public static string UnityBytesToStringASCII(this byte[] source)
	{
		// exit if null
		if (source.IsNullOrEmpty())
			return string.Empty;
		...
		using (MemoryStream compMemStream = new MemoryStream(source))
		{
			using (StreamReader reader = new StreamReader(compMemStream, Encoding.ASCII))
			{
				return reader.ReadToEnd();
			}
		}
